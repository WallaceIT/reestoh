$(window).load(function() {
    ...
    $("#mng_event_discount").change(function(){
        SQL += "UPDATE events SET `discount` = '"+$(this).val()+"' WHERE ID = "+eventID;
    ...
    $(document).on('click',".remove_cat",function(event){
        ...
        if (confirm('Eliminare compleamente la categoria e tutti i suoi prodotti?')) {
            ...
            if(!$(this).parent().parent().hasClass("newcat")){
                SQL += "DELETE FROM categories_"+evday+" WHERE ID = "+id+"§";
                SQL += "DELETE FROM items_"+evday+" WHERE category = "+id+"§";
            }
    ...
    $(document).on("click",".remove_item",function(event){
        ...
        if(!$(this).parent().parent().hasClass("mng_item_new")){
            SQL += "DELETE FROM items_"+evday+" WHERE ID = "+id+"§";
        };
    ...
    $("#mng_save").button({ icons: { secondary: "ui-icon-disk" } })
                  .click(function(){
                      ...
                      if(cat_order_changed == 1){
                          $(".group").each(function(){
                              SQL += "UPDATE categories_"+evday+" SET `displayorder` = '"+$(this).index()+"' WHERE ID = "+$(this).children().attr('cat')+"§";
                          });
                      }
                      // new items
                      ...
                      $(".mng_item_edited").each(function(){
                          if($('#mng_item_name_'+$(this).attr('item')).val()){
                              SQL += "UPDATE items_"+evday+" SET `name` = '"+$('#mng_item_name_'+$(this).attr('item')).val()+"', `price` = '"+$('#mng_item_price_'+$(this).attr('item')).val()+"' WHERE ID = "+$(this).attr('item')+"§";
                      ...
                      if(item_order_changed == 1){
                          $(".mng_item").each(function(){
                              SQL += "UPDATE items_"+evday+" SET `displayorder` = '"+$(this).index()+"' WHERE ID = "+$(this).attr('item')+"§";
                          });
                      }
